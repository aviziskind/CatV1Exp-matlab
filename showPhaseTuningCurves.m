...
for i = 1:length(inds)
    ...
    for cell_i = 1:length(cells)
        ...
        y = getPhaseTuningCurveFromOSP(data.OSP);
        ph = data.OSP.ph;
        plot(ph, y, 's-'); xlim([0 ph(end)])
        
        
        xlabel(['cell ' num2str(cells(cell_i))]);
    end 
%     matchAxes('C', h(1:length(cells)));
    
    suptitle_2(['Recording site ' num2str(Gid)] );
    input('');
