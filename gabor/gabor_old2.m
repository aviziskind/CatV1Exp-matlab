function y = gabor(A, mu_x, mu_y, sig_x, sig_y, theta, k, phi, X, gradientFlag) %#ok<INUSD>
    ...
    function G = gaborFunction(XY)
        XYp = shiftAndRotate(XY, theta, Xc);        
        Xp = XYp(1,:);
        Yp = XYp(2,:);
...
function Xp = shiftAndRotate(X, Xc, theta)  
    Xp = rotationMatrix(-theta) * (X - repmat(gaborCenter+Xc, 1,size(X,2)));
end
